...
(function(mod) {
    ...
    CodeMirror.defineMode("sql", function(config, parserConfig) {
        ...
        function pushContext(stream, state, type,isAddIndent) {
            // if(!!state.context){
            //   state.context.prev = state.context.type != ")"? {} :(state.context);
            //   state.context.indent =stream.indentation();
            //   state.context.col =stream.column();
            //   state.context.type =type;
            //   state.isAddIndent =isAddIndent;
            // }else{
            //   state.context = {
            //     prev: state.context,
        ...
        function pushContext2(stream, state, type,isAddIndent) {
            // if(!!state.context) {
            //   state.isAddIndent = isAddIndent;
            //   state.context.type = type;
            // }else{
            ...
            // }
            if(!!state.context) {
                if(state.context.type != type){
                    state.context.otherType = 'plain';
                }
                // state.isAddIndent = true;
                // state.context.type = type;
            }else{
        ...
        function popContext(state, isAddIndent) {
            state.indent = state.context.indent;
            state.context = state.context.prev;
            state.isAddIndent = false;
        }
        function popContext2(state, isAddIndent) {
            state.isAddIndent = false;
        }  // state.isAddIndent = isAddIndent;
        ...
        return {
            ...
            token: function(stream, state,isAddIndent) {
                if (stream.sol()) {
                ...
                if (tok == "(")
                    ...
                    pushContext2(stream, state, "plain", true);
                }
                else if(style == null && tok != ")" && tok != "]")
                    popContext2(state);
                else if (state.context && state.context.type == tok)
                    popContext(state, false);
                return style;
            },
            ...
            indent: function(state, textAfter) {
                ...
                // if(textAfter.charAt(0) == ')'&& cx.type == 'plain') {
                //   state.indent = cx.prev.indent;
                //   return cx.prev.indent;
                // }
                ...
                if (cx.align){
                    ...
                    console.log("bbb");
                    console.log(cx);
                    console.log(cx.type+'----'+textAfter.charAt(0));
                    console.log(cx.indent);
                    console.log(closing);
                    console.log(state.isAddIndent);
                    if(cx.type == 'plain'&&textAfter.charAt(0) == '('){
                        console.log('3333');
                        console.log(cx);
                        state.isAddIndent = false;
                    }else if(cx.type == ')' && textAfter.charAt(0) != '('&& !cx.otherType){
                        state.isAddIndent = false;
                    }
                    console.log("change");
                    console.log(state.isAddIndent);
                    return cx.indent + (closing ? 0 : (state.isAddIndent ? config.indentUnit:0 ));
                }
            },
    ...
    (function() {
        ...
        CodeMirror.defineExtension('sqlAddKeyWords', function (mode, str) {
            var strArray = [],
                mimeModesOrig = null;
