...
CodeMirror.defineMode("sql", function(config, parserConfig) {
  ...
  return {
    ...
    token: function(stream, state) {
      ...
      // $.inArray(tok, state.);
      // if(style == 'plain') {
      //   pushContext(stream, state, "plain");
      // }
      if (tok == "(")
        pushContext(stream, state, ")");
      else if (tok == "[")
        pushContext(stream, state, "]");
      else if (style == "keyword" && tok!= '('&& tok!=']') pushContext(stream, state, "plain");
    ...
    indent: function(state, textAfter) {
      ...
      console.error(textAfter.charAt(0));
      console.error(cx.type);
      console.log(cx);
...
//sql添加关键词－－静态方法
CodeMirror.addKeyWords =  function (mode, str) {
    var strArray = [],
      mimeModesOrig = null;
    if(arguments.length == 0) return;
    if(arguments.length > 1) {
      if(!!mode && typeof mode === 'string'&& this.mimeModes[mode] != null && typeof this.mimeModes[mode] === 'object') {
        mimeModesOrig = this.mimeModes[mode]
      } else {
        mimeModesOrig = this.mimeModes;
        console.error("Model does not exist or is illegal!!");
      }
    } else {
      str = mode;
    ...
    if(typeof str === "string") {
      strArray = str.split(" ");
      addAttr(mimeModesOrig);
      // for(var item in mimeModesOrig) {
      //   if(typeof mimeModesOrig[item] == 'object' && mimeModesOrig[item] != null) {
      //     if(!!mimeModesOrig[item]['keywords'] && typeof mimeModesOrig[item] == 'object') {
      //       for(var i=0;i <= strArray.length;i++) {
      //         if(!mimeModesOrig[item]['keywords'][strArray[i]]) {
      //           mimeModesOrig[item]['keywords'][strArray[i]] = true;
      //         }else {
      //           return;
      //         }
      //       }
